<template>
  <md-card class="card" v-show="!loading">
    <forecast-header :location="weather.location"></forecast-header>

    <md-card-content class="card__content" id="content">
      <img :src="weather.condition | iconWeather"
      ...
      <aside class="card__content--aside">
        ...
        <!--Primeiro dia-->
        <forecast-today :temp="weather.today"
                        :unit="weather.unit"
                        ...
                        :lat="lat"
                        :long="long"></forecast-today>

        <!--Segundo dia-->
        <forecast-tomorrow :unit="weather.unit"
                           :lat="lat"
                           :long="long"
                           :temperature="weather.tomorrow"></forecast-tomorrow>

        <!--Terceiro dia-->
        <forecast-last-day :unit="weather.unit"
                           :lat="lat"
                           :long="long"
                           :temperature="weather.after"></forecast-last-day>
      </aside>
    </md-card-content>
    ...
    methods: {
        setGradient(weather) {
            ...
            if (weather.today) {
                ...
                if (weather.unit === "C" && temp < 15) element.classList.add("card__content--background--blue");
                else if (weather.unit === "C" && temp > 35) element.classList.add("card__content--background--red");
